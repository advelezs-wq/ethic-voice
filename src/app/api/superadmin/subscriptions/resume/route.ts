...
export async function POST(req: NextRequest) {
  ...
  // Resume at provider (Mercado Pago) if configured and we have a preapproval id
  if (sub.providerSubscriptionId && mercadoPagoService.isConfigured()) {
    try {
      await mercadoPagoService.updatePreapproval(String(sub.providerSubscriptionId), { status: "authorized" });
    } catch (e) {
      console.warn("⚠️ [SUPERADMIN] Failed to resume MP preapproval", e);
    }
  }
